...
Tools and configuration for managing radio-related Raspberry Pis and a portable macOS packet radio setup. The Ansible project under `ansible/` manages:

- **marconi** — GPS-disciplined NTP server. Debian 12 (bookworm). Also runs [second-hand](https://github.com/arunderwood/second-hand), a web dashboard for chrony statistics.
...
Top-level layout: `ansible/` (active), `portable/` (standalone macOS setup for field-portable packet radio — not Ansible-managed), `legacy/` (archived ~2015 scripts, do not touch), `docs/` and `scripts/` (placeholders).
...
- `playbooks/marconi.yml` applies `common` + `gps_ntp` + `second_hand`. `playbooks/aloha.yml` applies `common` + `direwolf` + either `ax25` (kernel AX.25) or `linbpq` (LinBPQ node/BBS) — the two are substitutable, never both.
...
- `roles/linbpq/` runs LinBPQ as a packet radio node + BBS on aloha. LinBPQ has its own AX.25 and NET/ROM implementation — it does **not** use the kernel AX.25 stack. It connects directly to Direwolf's KISS TCP port (8001). The `linbpq` and `ax25` roles are **substitutable** — both provide "packet networking on top of Direwolf" but via different implementations. When `linbpq` is active, it defensively stops kissattach/netromd if present from a prior `ax25` deployment. LinBPQ is installed from a precompiled arm64 binary (`pilinbpq64` from G8BPQ) since the OARC apt repo doesn't publish arm64 packages for trixie. The binary lives in `/opt/linbpq/` alongside `bpq32.cfg` (LinBPQ reads config from its working directory, not `/etc/`). The BBS mail server is enabled via the `LINMAIL` directive; `linmail.cfg` is auto-generated on first run and configured via the web UI at port 8080. BBS callsign is W9CPZ-11 with alias ROOST. Secrets (sysop password) are stored in 1Password and resolved at runtime via the `community.general.onepassword` lookup plugin — never commit plaintext secrets to host_vars. The role also drops [BPQ-Alt-Webmail](https://github.com/jayflanzbaum-svg/BPQ-Alt-Webmail) (a single-file modern webmail UI + bundled fonts) into `/opt/linbpq/HTML/` when `linbpq_alt_webmail_enabled` is true (default). It's reachable at `http://aloha:8080/bpq-alt-webmail.html` alongside the stock UI; assets are pinned to an upstream release tag in `linbpq_alt_webmail_ref` so updates are a one-line variable bump. The two webmail `get_url` downloads (HTML, fonts) set `force: true` — the module otherwise skips the fetch whenever the destination already exists, which would make a ref bump a no-op. The LinBPQ binary is the opposite case: its URL is unversioned and serves G8BPQ's current build, so it is pinned by `linbpq_binary_checksum` with `force: false`. That combination is load-bearing — `get_url` only lets a matching destination checksum skip the download while `force` is false, so setting `force: true` there would turn the pin into a play that fails the moment upstream rebuilds. Upgrading LinBPQ means fetching the new binary, verifying it, and bumping the checksum.
- `roles/second_hand/` installs [second-hand](https://github.com/arunderwood/second-hand) on marconi — a FastAPI web dashboard for chrony sync statistics, reachable at `http://marconi:8000`. **There is no apt repository behind this package.** Upstream publishes `.deb` files only as GitHub Release assets, so `apt upgrade` (including the dist-upgrade in `common`) will never move it — upgrading means bumping `second_hand_version` in `defaults/main.yml` and re-running the playbook. The role discovers the target's architecture with `dpkg --print-architecture`, builds the asset URL from it, and installs via `apt: deb=<url>`; it skips the download entirely when the pinned version is already installed, so repeat runs never touch the network. Bind address and port are applied as a systemd drop-in (`/etc/systemd/system/second-hand.service.d/override.conf`) rather than by editing the packaged unit — that drop-in carries nothing else, since the packaged unit handles its own chrony ordering and systemd hardening. **Before bumping the version, confirm the tag actually has assets.** Upstream's release workflow has twice published tags with no artifacts at all — a broken amd64 `test-deb` leg emptied v2.0.0 through v2.1.0, and a lintian gate that only runs on release tags emptied the first v3.0.0. Both are fixed, and v3.0.0 is the currently pinned version. Check before bumping with `gh release view v<version> --repo arunderwood/second-hand --json assets`. Releases from v2.1.1 on ship two assets (amd64 + arm64); older ones also carried `dbgsym` packages, which the role must never select.
...
- `portable/` (top-level, outside `ansible/`) is a standalone macOS setup for field-portable packet radio with an HT + AIOC. It is NOT Ansible-managed — it contains a static `direwolf.conf`, a device-discovery script (`find-aioc.sh`), and a launcher (`start.sh`) that auto-detects the AIOC and starts Dire Wolf. Paracon (zero-dependency `.pyz` packet terminal) connects via Dire Wolf's AGW port (8000). No kernel AX.25 on macOS — everything runs in userspace.
- `tests/` holds ansible-native assert playbooks that run offline: `boot_regex.yml` exercises the lineinfile regexes from `roles/gps_ntp/tasks/boot.yml`, `direwolf_conf.yml` renders `roles/direwolf/templates/direwolf.conf.j2` and asserts required directives are present and Windows-only tokens are absent, `ax25_conf.yml` renders the axports/nrports/nrbroadcast templates and asserts the expected lines are produced, `linbpq_conf.yml` renders `bpq32.cfg.j2` and asserts required BPQ directives are present, and `second_hand_conf.yml` renders the second-hand systemd drop-in and asserts the release-asset name/URL built from the role defaults still match what upstream publishes. Add new test playbooks here — `make test` globs `tests/*.yml` so they wire in automatically.
